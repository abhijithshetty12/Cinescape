...
import { Bookmark, Search, SortAsc, SortDesc, Filter, ImageOff, Star } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    try {
      ...
      const unsubscribe = onSnapshot(q,
        async (snapshot) => {
          const fetchedWatchlist = await Promise.all(snapshot.docs.map(async (doc) => {
            const data = doc.data();
    ...
    return (
      <div className="container mx-auto mt-8 px-4">
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-red-400 mb-2">Error Loading Watchlist</h3>
    ...
    <div className="container mx-auto mt-8 px-4">
      <div className="flex items-center gap-3 mb-6">
        ...
        <span className="text-gray-400 text-lg">
          ({filteredAndSortedWatchlist.length} {mediaType === 'movie' ? 'movies' : 'series'})
        </span>
      </div>
      ...
      {filteredAndSortedWatchlist.length === 0 && !loading && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-400 mb-2">
            No {mediaType === 'movie' ? 'movies' : 'series'} in watchlist yet
          </h3>
          <p className="text-gray-500 mb-4">
            Start adding some {mediaType === 'movie' ? 'movies' : 'series'} to your watchlist!
          </p>
          <Link
            to="/trending"
      ...
      {filteredAndSortedWatchlist.length > 0 && (
        <div className="text-center mt-8">
          <p className="text-gray-400 text-sm">
            Showing {filteredAndSortedWatchlist.length} {mediaType === 'movie' ? 'movies' : 'series'}
          </p>
        </div>
      )}
    </div>
